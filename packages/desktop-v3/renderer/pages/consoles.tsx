...
export default function HomePage() {
  ...
  return (
    <React.Fragment>
      ...
      <PageHeader title="My Consoles" subtitle="Manage and stream from your home consoles" />
              ...
              <div className="flex items-start justify-between">
                ...
                <div className="flex items-center gap-3.5">
                  <div>
                    ...
                    <p className="text-white/40 text-xs mt-0.5">{device.type}</p>
                  </div>
                </div>

                <div className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold ${device.powerState === 'On' ? 'bg-[#107C10]/20 text-[#107C10] border border-[#107C10]/30' : 'bg-white/5 text-white/35 border border-white/10'}`}>
                  {device.powerState === 'On' ? 'Online' : 'Standby'}
                </div>
