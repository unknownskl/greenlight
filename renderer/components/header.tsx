    ...
    for(const link in links){
      linksHtml.push(<li key={ link }>
        <Link href={ links[link].url }>
          <a title={ links[link].title } onClick={ () => {setMenuActive(link)} } className={links[link].active === true ? 'active' : ''}>{ links[link].name }</a>
        </Link>
      </li>)
    }
  ...
  return (
    <React.Fragment>
        <div id="component_header" className={hidden === true ? 'disabled' : ''}>
          <a onClick={ () => { if(confirm('Close xbox-xcloud-client?')){window.close();} } } id="actionBarLogo" title="Home">
